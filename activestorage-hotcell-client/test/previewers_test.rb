...
class PreviewersTest < ActiveStorageHotCellClientTest
  ...
  # not inherited.
  MUPDF = [ ActiveStorage::HotCell::PdfPreviewer, ActiveStorage::Previewer::MuPDFPreviewer ].freeze
  FFMPEG = [ ActiveStorage::HotCell::VideoPreviewer, ActiveStorage::Previewer::VideoPreviewer ].freeze
  ...
  def test_accept_does_not_care_whether_the_binary_is_installed
    with_binary_missing MUPDF, :@mutool_exists do
      assert ActiveStorage::HotCell::PdfPreviewer.accept?(Blob.new(fixture("sample.pdf")))
    end

    with_binary_missing FFMPEG, :@ffmpeg_exists do
      assert ActiveStorage::HotCell::VideoPreviewer.accept?(Blob.new(fixture("sample.mp4")))
    end
  end
  ...
  def test_the_accepted_content_types_are_the_ones_rails_accepts
    refute ActiveStorage::HotCell::PdfPreviewer.accept?(Blob.new(fixture("colour.png")))
    refute ActiveStorage::HotCell::VideoPreviewer.accept?(Blob.new(fixture("sample.pdf")))
    assert ActiveStorage::HotCell::VideoPreviewer.accept?(Blob.new(fixture("sample.mp4")))
  end

  def test_a_pdf_preview_yields_what_rails_yields
    with_cell do
      preview ActiveStorage::HotCell::PdfPreviewer, "sample.pdf" do |attachable|
  ...
  def test_a_video_preview_yields_what_rails_yields
    with_cell do
      preview ActiveStorage::HotCell::VideoPreviewer, "sample.mp4" do |attachable|
  ...
  def test_options_are_passed_through_to_the_attachable
    with_cell do
      preview ActiveStorage::HotCell::PdfPreviewer, "sample.pdf", service_name: "somewhere" do |attachable|
  ...
  def test_an_undecodable_document_raises_the_permanent_class
    with_cell do
      assert_raises Unprocessable do
        preview(ActiveStorage::HotCell::PdfPreviewer, "broken.pdf") { flunk "should not have yielded" }
      end
    end
  end
